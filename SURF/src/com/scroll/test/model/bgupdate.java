...
import com.surf.command.BbgupdateCommand;


@WebServlet("/bgupdate.do")
public class bgupdate extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if(orginFile==null) {
				orginFile="img/bg.png";
			}else {
				orginFile = "file_upload/"+filename; //같은 이름이지만 내용이 다른 파일 일 수 있기 때문에 바뀐파일명을 넣어준다.
		...
		/*	System.out.println(orginFile); //이미지
			...
			command = new BbgupdateCommand();       // 업데이트를 먼저 시켜줌 디비가 변경
