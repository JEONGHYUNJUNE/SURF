...
<%@ page import="java.util.*" %>
  <c:if test = "${loginUser.userid eq fuser.userid}">
    <jsp:forward page="info.do?userid=${loginUser.userid }"/>
    </c:if>  
...
<input type="hidden" value="${loginUser.username }" id="loginuserid2">
   <div class="divmain" id="mainContent"> <!-- 메인 피드의 게시물 내용이 담김  -->
   <div class="myinfo"  style="color: white; background-image: url(${fuser.profilebg})  "> <!-- 내 배경 프로필 및 정보  -->
        <img  onclick = "picfullmodal(this)" src="${fuser.profilepic}" width="80" height="80" style="cursor:pointer;    background-size :cover;  margin-left:3%;  border-radius: 50%;border: 1.5px solid white ">
         <span style="font-size: 30px;position: absolute; margin-top: 1%;margin-left: 1%" >${fuser.username}</span>
         <br>
  ...
  <!-- <p class="pfchange" onclick="picclick()" >프로필 수정</p>  --><br><Br>

<!--   <p class="pfchange" onclick="asd1()" >프로필 삭제</p> <br>  -->
 <input type="submit" id="picchangego" value="기본이미지로" style=display:none; >
 <input name="userid" value="${fuser.userid}" style="display: none">
 </form>
       ...
       <input type="file" name="bgfile" onchange="asd2()" value="" id="bgfile" style=" size: 50% ;display:none;"> <!--파일변경시 바로 서블릿 적용  -->
                    <% int friendnotnum = 0; %>
                      <% int friendnum = 0; %>
                  <c:forEach items="${friendalllist}" var="friend">
                     <% friendnum++; %>
                   <c:if test="${friend.userid ne fuser.userid }">
                      <% friendnotnum++; %>
                </c:if>
             </c:forEach>
<% if(friendnotnum==friendnum){ %>
<input type="button" class="bgchange" 
style="border:1px solid white ;background-color: transparent; padding: 8px; margin-bottom: 10px"
 onclick="addfriend1()" value="친구신청"> 
 <input id="fffuserid" value="${fuser.userid}" style="display: none">
 <% };%>
           <c:forEach items="${friendlist}" var="friend">
           <c:if test="${friend.userid eq fuser.userid }">
       <input type="button" class="bgchange" 
style="border:1px solid white ;background-color: transparent; padding: 8px; margin-bottom: 10px"
 onclick="fdel1()" value="친구삭제"> 
 <input id="senduserid" value="${fuser.userid}" style="display: none">
 </c:if>
 </c:forEach>
           <c:forEach items="${sendlist}" var="friend">
           <c:choose>
       <c:when test="${friend.userid eq fuser.userid }">
<input type="button" class="bgchange" 
style="border:1px solid white ;background-color: transparent; padding: 8px; margin-bottom: 10px"
 onclick="fdel1()" value="신청취소"> 
  <input id="senduserid" value="${fuser.userid}" style="display: none">
 </c:when>
 <c:otherwise>
 </c:otherwise>
  ...
  <c:forEach items="${receivelist}" var="friend">
       ...
       <input type="button" class="bgchange" 
style="border:1px solid white ; background-color: transparent; padding: 8px; margin-bottom: 10px"
 onclick="window.location='friendadd2.do?loginid=${loginUser.userid}&fid=${friend.userid}'" value="친구신청 수락"> 
 </c:if>
</c:forEach>
          <input name="fname" id='fname' value="${fuser.username}" style="display: none">
          <input name="userid" id='fid' value="${fuser.userid}" style="display: none">
   ...
   function addfriend1(){
      var fid = $("#fid").val();
       var loginuserid =$("#loginuserid1").val();
       var loginusername =$("#loginuserid2").val();
       var fname = $('#fname').val();
       window.location='friendadd1.do?loginid='+loginuserid+'&fid='+fid+'&fname='+fname+'&loginusername='+loginusername;
   }
   function fdel1(){
        var fid = document.getElementById("senduserid").value;
   ...
   <div class="info">
      ...
      <c:if test="${dto.userid eq fuser.userid}">
      
         <div class="divmain-content" style="float: left;">
            <div class="divmain-content-top">
               <div class="pf" id="pf" style="background-image : url(${fuser.profilepic}) "></div> <!--프로필 사진 구역  -->
               <div class="namedate">
                  <b>${dto.name}</b><br> 
                    <c:choose>
  ...
  <p id="linkinsert"> 삽입 링크 : <A id="linkexist" href="http://${dto.link}" target="_blank">${dto.link}</A></p>
      ...
      <c:otherwise>
         ...
         <input type="hidden" value="<%=b %>" id="iffriend">       
      <% if(b==0){ %> <!--b가 0이면 친구가 아니다  -->
      
       <c:forEach items="${list2}" var="dto"> <!--for 문을 통해 dto의 게시물 내용을 불러옴  -->
      <c:if test="${dto.userid eq fuser.userid}">
         <div class="divmain-content" style="float: left;">
            <div class="divmain-content-top">
               <div class="pf" id="pf" style="background-image : url(${fuser.profilepic}) "></div> <!--프로필 사진 구역  -->
  ...
  <p id="linkinsert"> 삽입 링크 : <A id="linkexist" href="http://${dto.link}" target="_blank">${dto.link}</A></p>
    ...
    <P style="font-size: 16x;font-family:fantasy; text-align: center;"> ${fuser.username}의 서핑 지역  
    </P><center id="locinfo" style="font-size:50px ;font-family: 'Nanum Pen Script', cursive;">${fuser.surfaddress}</center>
      ...
      </select>&nbsp;<input type="button" onclick="locch()" value="변경"> -->
...
<script src="http://vjs.zencdn.net/c/video.js"></script>
 ...
 <script src="http://code.jquery.com/jquery-3.1.1.min.js"></script> 
 <script type="text/javascript" src="js/friendpage.js">
</script>
<script type="text/javascript">
