...
$(window).scroll(function() {
   if ($(window).scrollTop() >= $(document).height() - $(window).height()-4&&attr) {
          attr= false;
          request.open('Post','./test1.do?num='+num,true);
...
var anum = 8;
 function Scroll(){
   ...
   if(request.readyState == 4 && request.status == 200){
      ...
      var ajaxprofilepic = document.getElementById("ajaxprofilepic").value; 
      var ajaxusername = document.getElementById("ajaxusername").value;
      if(result.length!=0){
            ...
            num1 = num1 + 1;
             ...
             if(result[i][2] != "null"){
              ...
              $('#mainContent').append(
                    ...
                    +'<div  class="pf" id="pf'+anum+'" style="background-image : url('+dispic+');"></div>'
                    +'<div class="namedate">'
                      ...
                      + '<div id="likecount'+anum+'" style="font-size:14px;width:70px;height:30px;float:left;padding-left:20px;"><b>좋아요</b><span style="padding-left:3px;">0</span></div>'
                      + '<div id="replycount'+anum+'" style="font-size:14px;width:60px;height:30px;float:left;padding-left:5px;"><b>댓글</b><span style="padding-left:3px;" id="replysu'+anum+'">0</span></div>'
                      + '<input type="hidden" value="0" id="replycountnum'+anum+'">'
                      + '<table id="replyform'+anum+'" width="280px">'
                      + '</table>'
                       +   '<input type="hidden" value="'+ajaxuserid+'" id="replyuserid'+anum+'">'
                       +   '<input type="hidden" value="'+ajaxusername+'" id="replyusername'+anum+'">'
                       +   '<input type="hidden" value="'+result[i][4]+'" id="replybnum'+anum+'">'
                       +   '<input type="text" placeholder="댓글을 입력하세요." id="replycontent'+anum+'" class="replyinpt">'
                       +   '<input type="button" value="전송" class="replybtn" onclick="replywrite1('+anum+')">'
                       +' </div></div>'
              );
                anum = anum+1;
                videodel(result[i][4]);
      ...
      //webfeed.jsp 에서 받아온 로그인 유저의 프로필사진
         for(var z = (anum-num1);z<anum;z++){
            replyajax(z);
         }
         profilechg1(anum,num1);
...
$(function(){
   ...
   function profilechg1(num,num1){
         for(var i = (num-num1);i<num;i++){
                   $.ajax({
                       type:"GET",
                       url:'profile.do?userid='+document.getElementById("boarduserid"+i).value,
                       datatype:"TEXT",
